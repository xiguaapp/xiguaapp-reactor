package com.cn.xiguaapp.r2dbc.orm.rdb.support.commons;
...
import com.cn.xiguaapp.r2dbc.orm.rdb.mapping.defaults.record.RecordResultWrapper;
...
import com.cn.xiguaapp.r2dbc.orm.rdb.metadata.parser.TableMetadataParser;
...
public abstract class RDBTableMetadataParser implements TableMetadataParser {
    ...
    protected Optional<RDBTableMetadata> doParse(String name) {
        ...
        //列
        List<RDBColumnMetadata> metaDataList = getSqlExecutor().select(SqlRequests.template(getTableMetaSql(name), param),
                ResultWrappers.list(new RDBColumnMetaDataWrapper(metaData)));
        metaDataList.forEach(metaData::addColumn);
        //说明
        Map<String, Object> comment = getSqlExecutor().select(SqlRequests.template(getTableCommentSql(name), param), singleMap());
        ...
        return tableExistsReactive(name)
                ...
                .flatMap(ignore -> {
                    ...
                    Mono<List<RDBColumnMetadata>> columns = reactiveSqlExecutor
                            .select(SqlRequests.template(getTableMetaSql(name), param), new RDBColumnMetaDataWrapper(metaData))
                            .doOnNext(metaData::addColumn)
                            .collectList();
                    //注释
                    Mono<Map<String, Object>> comments = reactiveSqlExecutor
                            .select(SqlRequests.template(getTableCommentSql(name), param), singleMap())
    ...
    public boolean tableExists(String name) {
        ...
        return getSqlExecutor()
                .select(SqlRequests.template(getTableExistsSql(), param),
                        ResultWrappers.optional(ResultWrappers.single(ResultWrappers.column("total", Number.class::cast))))
    ...
    public List<String> parseAllTableName() {
        return getSqlExecutor()
                .select(SqlRequests.template(getAllTableSql(), Collections.singletonMap("schema", schema.getName())), ResultWrappers.list(ResultWrappers.column("name", String::valueOf)));
    }
    ...
    protected Flux<RDBTableMetadata> fastParseAllReactive() {
        ...
        Mono<Void> columns = getReactiveSqlExecutor()
                .select(SqlRequests.template(getTableMetaSql(null), param), new RecordResultWrapper())
                .doOnNext(record -> {
        ...
        Flux<Record> comments = getReactiveSqlExecutor()
                .select(SqlRequests.template(getTableCommentSql(null), param)
    ...
    protected List<RDBTableMetadata> fastParseAll() {
        ...
        getSqlExecutor()
                .select(
                        SqlRequests.template(getTableMetaSql(null), param), ResultWrappers.
        ...
        getSqlExecutor()
                .select(SqlRequests.template(getTableCommentSql(null), param)
                        , ResultWrappers.consumer(new RecordResultWrapper(), record -> {
