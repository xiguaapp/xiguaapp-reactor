...
import com.cn.xiguaapp.common.oauth.core.annotation.SaCheckLogin;
import com.cn.xiguaapp.common.oauth.core.annotation.SaCheckPermission;
import com.cn.xiguaapp.common.oauth.core.annotation.SaCheckRole;
...
import com.cn.xiguaapp.common.oauth.core.exception.NotLoginException;
import com.cn.xiguaapp.common.oauth.core.exception.NotPermissionException;
import com.cn.xiguaapp.common.oauth.core.exception.NotRoleException;
import com.cn.xiguaapp.common.oauth.core.function.SaFunction;
import com.cn.xiguaapp.common.oauth.core.manage.SaTokenManager;
import com.cn.xiguaapp.common.oauth.core.model.SaLoginModel;
import com.cn.xiguaapp.common.oauth.core.model.SaTokenInfo;
import com.cn.xiguaapp.common.oauth.core.model.TokenSign;
...
public class StpLogic {
    ...
    public void setLoginId(Object loginId, SaLoginModel loginModel) {
        ...
        HttpServletRequest request = SaTokenManager.getSaTokenServlet().getRequest();
        SaTokenConfig config = getConfig();
        SaTokenService dao = SaTokenManager.getSaTokenDao();
    ...
    public Object getLoginIdDefaultNull() {
        ...
        // 如果已经[临时过期] 
        if(getTokenActivityTimeoutByToken(tokenValue) == SaTokenService.NOT_VALUE_EXPIRE) {
            return null;
        }
    ...
    protected void setLastActivityToNow(String tokenValue) {
        ...
        if(tokenValue == null || getConfig().getActivityTimeout() == SaTokenService.NEVER_EXPIRE) {
            return;
        }
        // 将[最后操作时间]标记为当前时间戳 
    ...
    public void checkActivityTimeout(String tokenValue) {
        ...
        if(timeout == SaTokenService.NEVER_EXPIRE) {
            return;
        }
        // -2 代表已过期，抛出异常 
        if(timeout == SaTokenService.NOT_VALUE_EXPIRE) {
            throw NotLoginException.newInstance(loginKey, NotLoginException.TOKEN_TIMEOUT);
        }
        // --- 至此，验证已通过 
    ...
    public long getTokenActivityTimeoutByToken(String tokenValue) {
        ...
        // 如果设置了永不过期, 则返回 -1 
        if(getConfig().getActivityTimeout() == SaTokenService.NEVER_EXPIRE) {
